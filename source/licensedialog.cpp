...
LicenseDialog::LicenseDialog(License *license, Settings *settings, QWidget *parent /* Q_NULLPTR */, Qt::WindowFlags flags /* 0 */) : QDialog(parent, flags), _refreshed(false), _license(license), _settings(settings)
{
  _ui.setupUi(this);
...
{
  const auto fileName = QFileDialog::getOpenFileName(this, tr("Load license"), QString(), tr("license (*.lic)"));
  if (fileName.isEmpty())
  {
    return;
  }
  ...
  const auto licenseData = file.readAll();
