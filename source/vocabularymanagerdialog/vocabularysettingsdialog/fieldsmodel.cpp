...
{
  const auto sourceFieldId      = _vocabulary->fieldId(sourceRow);
  const auto destinationFieldId = _vocabulary->fieldId(destinationRow);
...
bool FieldsModel::setData(const QModelIndex &index, const QVariant &value, int role /* Qt::EditRole*/)
{
  ...
  switch (index.column())
  {
    ...
    case Column::Speech:
      {
      auto fieldAttributes = _vocabulary->fieldAttributes(fieldId);
